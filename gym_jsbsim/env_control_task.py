...
class TaxiControlTask(BaseFlightTask):
    ...
    def get_initial_conditions(self) -> Dict[Property, float]:
        ...
        initial_conditions = {prp.initial_altitude_ft: self.INITIAL_ALTITUDE_FT,
                              ...
                              prp.shortest_ac_to_path: self.shortest_ac_dist(self.INIT_LAT, self.INIT_LON, self.PATH[0][1], self.PATH[0][0], self.PATH[1][1], self.PATH[1][0])
                             }
        return initial_conditions
    ...
    def dist_points(self, lat1,lon1,lat2,lon2):
        '''
        compute the distance between two gps point in m
    ...
    def shorter_dist_point_circle(self, p_lat, p_lon, c_lat, c_lon, c_radius):
        ''' 
        compute shorter distance between path point P(x1,x2) and the circle of center C(h,k) and radius r (in m)
        abs(sqrt((x1 - h)² + (y1 - k)²) - r)
            
        >>> round(shorter_dist_point_circle(40.759752, -73.974215, 40.758492, -73.975105, 8))
        7
        >>> round(shorter_dist_point_circle(40.759809, -73.976264, 40.758492, -73.975105, 8))
        10
        '''
    ...
    def _get_reward(self, sim: Simulation, last_state: NamedTuple, action: NamedTuple, new_state: NamedTuple) -> float:
        ...
        shortest_point_dist = 9999
        shortest_path_dist = 9999
        for i in range(0, len(self.PATH)-1):
            # find the closer point to the aircraft
            point_dist = self.dist_points(lat, lon, self.PATH[i][1], self.PATH[i][0])
            if (point_dist < shortest_point_dist):
                shortest_point_dist = point_dist
                id_path_closer_point = i

            # For point that are forehead (to compute the good intersection of the circle)
            if (math.fabs(sim[prp.heading_deg] - self.calculate_initial_compass_bearing((lat,lon), (self.PATH[i][1],self.PATH[i][0])))<180):
        ...
        print(f'heading: {sim[prp.heading_deg]}, target heading: {sim[prp.target_heading_deg]}, delta_heading: {sim[prp.delta_heading]}, radius: {action.radius_circle}')
