...
class HeadingControlTask(Task):
    ...
    def get_reward(self, state, sim):
        ...
        heading_r = math.exp(-(sim.get_property_value(c.delta_heading) /
                               heading_error_scale)**2)

        alt_error_scale = 50.  # feet
        alt_r = math.exp(-(sim.get_property_value(c.delta_altitude) /
                           alt_error_scale)**2)

        roll_error_scale = 0.35  # radians ~= 20 degrees
        roll_r = math.exp(-(sim.get_property_value(c.attitude_roll_rad) /
                            roll_error_scale)**2)

        speed_error_scale = 16  # fps (~5%)
        speed_r = math.exp(-((sim.get_property_value(c.velocities_u_fps) -
                              800)/speed_error_scale)**2)
    ...
    def is_terminal(self, state, sim):
        ...
        if sim.get_property_value(c.simulation_sim_time_sec) >= sim.get_property_value(c.steady_flight):
            ...
            new_heading = sim.get_property_value(
                c.target_heading_deg) + sign * angle
            ...
            sim.set_property_value(
                c.steady_flight, sim.get_property_value(c.steady_flight)+150)
